...
unsigned int copy_DIR_out(string distination,FAT_Class::Link target,File *myfile,FAT_Class *fat){
	...
	if(mkdir(FullPathWithName.c_str(),0777)<0){
		cout<<distination<<": no such file or directory"<<endl;
		return 0;
	}
	FAT_Class::Link temp;
...
inode_Class* touch(const char * filename,FAT_Class *FAT){
	...
	if(!temp){
		cout<<"touch: cannot create '"<<filename<<"' a file or directory with the same name exists choose another name for the file"<<endl;
		return NULL;
	}
	else
		return	&(temp->inode);
...
inode_Class* mkdir(const char * filename,FAT_Class *FAT){
	...
	if(!node){
		cout<<"mkdir: cannot create '"<<filename<<"' a file or directory with the same name exists choose another name for the directory"<<endl;
		return NULL;
		}
	else
		return	node;
